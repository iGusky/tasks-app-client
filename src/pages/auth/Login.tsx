...
export const Login = () => {
  ...
  const handleLogin = async (values: any) => {
    try {
      ...
      toast.error(e.response.data.message || "Ocurrió un error al inciar sesión")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className='max-w-[400px] m-auto p-2'>
        <form className='flex flex-col gap-2 p-4 rounded border' onSubmit={form.onSubmit((values) => handleLogin(values)) }>
          ...
          <Link to="/auth/signup" className='flex justify-center'>
            <Button variant='transparent'>¿No tienes cuenta? Crea una ahora</Button>
          </Link>
        </form>
    </div>
  )
